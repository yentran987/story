...
export async function characterLogicCheck(text: string, profile: string, language: 'auto' | 'en' | 'vi' = 'auto') {
  ...
  try {
    ...
    if (parsed.issues && Array.isArray(parsed.issues)) {
      const explanation = parsed.issues.length ? `Detected ${parsed.issues.length} potential inconsistencies for the provided character profile.` : 'No clear issues detected.';
      const fix_suggestions = parsed.issues.map((i: any) => i.fix || 'Adjust the scene to better match profile (add motivation or internal conflict).');
      return { language: lang, issues: parsed.issues.map((i: any) => i.reason || JSON.stringify(i)), explanation, fix_suggestions };
    }
  } catch (e) {
    // ignore parse error and fallback
  }

  // fallback: return empty result with explanation
  const explanation = 'No clear issues detected.';
  return { language: lang, issues: [], explanation, fix_suggestions: [] };
}
...
export async function generateHashtags(text: string, language: 'auto' | 'en' | 'vi' = 'auto') {
  ...
  try {
    const parsed = JSON.parse(out || '{}');
    if (parsed.tags && Array.isArray(parsed.tags)) return { language: lang, hashtags: parsed.tags.slice(0,15) };
  } catch { }

  return { language: lang, hashtags: ['#drama','#adventure','#romance','#fantasy','#mystery'] };
}
...
export async function moderateContent(text: string, language: 'auto' | 'en' | 'vi' = 'auto') {
  ...
  return { language: lang, issues, risk_score: risk, recommended_fix: suggestion.trim() };
}
...
export async function summarizeStory(fullText: string, language: 'auto' | 'en' | 'vi' = 'auto') {
  ...
  try {
    const parsed = JSON.parse(out || '{}');
    if (parsed.summary) {
      const main_events = parsed.main_events || [];
      const character_arcs = parsed.character_arcs || [];
      return { language: lang, summary: parsed.summary, main_events, character_arcs };
    }
  } catch {}

  return { language: lang, summary: lang === 'vi' ? 'Tóm tắt ngắn gọn — cốt truyện, bước ngoặt và kết luận.' : 'Short summary: plot, turning points, and conclusion.', main_events: [], character_arcs: [] };
}
...
export async function fastRecap(chapterText: string, language: 'auto' | 'en' | 'vi' = 'auto') {
  ...
  try {
    const parsed = JSON.parse(out || '{}');
    if (parsed.bullets && Array.isArray(parsed.bullets)) return { language: lang, fast_recap: parsed.bullets.slice(0,6) };
    if (parsed.fast_recap && Array.isArray(parsed.fast_recap)) return { language: lang, fast_recap: parsed.fast_recap.slice(0,6) };
  } catch {}

  // Try to make a useful fallback by extracting the first 3 sentences from the chapter
  const quickExtract = (text: string) => {
    if (!text || text.trim().length === 0) return [];
    // split by sentence enders, keep non-empty
    const sents = text.split(/(?<=[.!?])\s+/).map(s=>s.trim()).filter(Boolean);
    if (sents.length === 0) return [];
    // return up to 3 compact sentences as bullets
    return sents.slice(0, 3).map(s => s.length > 120 ? s.slice(0, 117).trim() + '...' : s);
  };

  const bullets = quickExtract(chapterText || '');
  if (bullets.length) return { language: lang, fast_recap: bullets };

  return { language: lang, fast_recap: lang === 'vi' ? ['Sự kiện 1', 'Sự kiện 2'] : ['Event 1', 'Event 2'] };
}
