...
export const Editor: React.FC<EditorProps> = ({ setView, onBack, showToast, initialStory, onSaveStory }) => {
  ...
  const handleDeleteChapter = (chapterId: string) => {
    ...
    setIsAiLoading(true);
        ...
        if (type === 'twist') {
            const result = await geminiService.suggestPlotTwists(context, aiLanguage);
            setAiSuggestion((result.twists && result.twists[0]) || '');
        } else {
            // rewrite default to 'soft' tone
            const res = await geminiService.rewriteSection(context || title, 'soft', aiLanguage);
            setAiSuggestion(res.rewritten || '');
        }
    ...
    const handleGenerateSynopsis = async (keywordsInput?: string) => {
        ...
        setIsAiLoading(true);
        const res = await geminiService.generateSynopsis(keywords, aiLanguage);
        setAiSynopsis(res.synopsis || null);
        setIsAiLoading(false);
    };

    const handleSuggestTitles = async () => {
        const res = await geminiService.suggestChapterTitles(activeChapter.content || title, aiLanguage);
    ...
    const handleCharacterCheck = async () => {
        ...
        const res = await geminiService.characterLogicCheck(activeChapter.content || title, parsed, aiLanguage);
    ...
    <div className="flex h-[calc(100vh-65px)] overflow-hidden bg-white text-slate-800 font-sans">
      ...
      {showAiPanel && (
          <div className="w-80 bg-white border-l border-slate-200 flex flex-col flex-shrink-0 shadow-xl z-20">
              <div className="p-5 border-b border-slate-100 bg-purple-50/30 flex items-center justify-between">
