...
import { suggestChapterTitles, characterLogicCheck as aiCharacterLogicCheck } from '../services/aiService';
...
export const Editor: React.FC<EditorProps> = ({ setView, onBack, showToast, initialStory, onSaveStory }) => {
  ...
  const [coverUrl, setCoverUrl] = useState(initialStory?.coverUrl || 'https://picsum.photos/300/450');
    ...
    const [chapters, setChapters] = useState<Chapter[]>(() => {
        const initial = initialStory?.chapters;
  ...
  useEffect(() => {
    if (initialStory) {
        ...
        setCoverUrl(initialStory.coverUrl);
                // ensure there's at least one chapter when editing
                if (initialStory.chapters && initialStory.chapters.length > 0) {
                    setChapters(initialStory.chapters);
                    setActiveChapterId(initialStory.chapters[0].id);
                } else {
                    const newChapterId = Date.now().toString();
                    const initialCh = [{ id: newChapterId, title: 'Chapter 1: The Beginning', content: '' }];
                    setChapters(initialCh);
                    setActiveChapterId(newChapterId);
                }
    } else {
        // Reset for new story (leave id blank so DB will create a new row)
        setStoryId('');
  ...
  const [showAiPanel, setShowAiPanel] = useState(true);
    const [aiLanguage, setAiLanguage] = useState<'auto' | 'en' | 'vi'>('auto');
  ...
  const [isAiLoading, setIsAiLoading] = useState(false);
    ...
    const [aiCharacterCheck, setAiCharacterCheck] = useState<{ issues: string[]; explanation?: string; fix_suggestions?: string[] } | null>(null);
  ...
  const handleContentChange = (newContent: string) => {
    ...
    // Refs for the AI panel / suggestion display — used to scroll results into view so they're not hidden by UI chrome
  ...
  const handleDeleteChapter = (chapterId: string) => {
      ...
      if (window.confirm("Are you sure you want to delete this chapter?")) {
        ...
        try {
            // prefer the older helper from the files you sent: generateStoryIdeas(context, type)
            const res = await geminiService.generateStoryIdeas(context, type);
            // may return string or object. Prefer string results.
            // generateStoryIdeas returns a string in this implementation
            setAiSuggestion(res || '');
        } catch (err) {
            console.warn('generateStoryIdeas failed', err);
            setAiSuggestion('Could not generate idea.');
        }
        setTimeout(() => suggestionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50);
        setIsAiLoading(false);
    };

    const handleGenerateSynopsis = async (keywordsInput?: string) => {
        const keywords = keywordsInput ? keywordsInput.split(/,|;/).map(k => k.trim()).filter(Boolean) : title.split(/\s+/).slice(0,3);
        if (keywords.length === 0) { showToast('Provide some keywords', 'error'); return; }
        setIsAiLoading(true);
        // generateStoryIdeas supports 'outline' / 'synopsis' in the simpler geminiService
        const kws = Array.isArray(keywords) ? keywords.join(', ') : String(keywords);
        const res = await geminiService.generateStoryIdeas(kws, 'outline');
        setAiSynopsis(res || null);
        setTimeout(() => suggestionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50);
        setIsAiLoading(false);
    };

    const handleSuggestTitles = async () => {
        const res = await suggestChapterTitles(activeChapter.content || title, aiLanguage);
        setAiTitles(res.titles || []);
        setTimeout(() => suggestionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50);
    };

    const handleCharacterCheck = async () => {
        const profile = window.prompt('Paste the character profile JSON or short description:');
        if (!profile) { showToast('Character profile required', 'error'); return; }
        setIsAiLoading(true);
        const parsed = (() => {
            try { return JSON.parse(profile); } catch { return profile; }
        })();
        const res = await aiCharacterLogicCheck(activeChapter.content || title, parsed, aiLanguage);
        setAiCharacterCheck({ issues: res.issues || [], explanation: res.explanation, fix_suggestions: res.fix_suggestions });
        setTimeout(() => suggestionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50);
        setIsAiLoading(false);
    };
  ...
  const handleModeration = async () => {
      ...
      setIsAiLoading(true);
            ...
            const result: any = await geminiService.moderateContent(activeChapter.content);
            // support legacy string response (e.g. 'Approved') and new structured object
            if (typeof result === 'string') {
                if (result.includes('Approved')) {
                    setModerationResult('approved');
                    setModerationMessage(result || null);
                    showToast(aiLanguage === 'vi' ? 'Nội dung an toàn!' : 'Content looks safe!', 'success');
                } else {
                    setModerationResult('flagged');
                    setModerationMessage(result || 'Potential issues found.');
                    showToast(aiLanguage === 'vi' ? 'Phát hiện vấn đề — kiểm tra chi tiết trong bảng thông báo.' : 'Potential issues found. See details.', 'error');
                }
            } else {
                // structured response
                const issues = result?.issues || [];
                const risk = (result as any)?.risk_score || (result as any)?.risk || 'low';
                if (issues.length === 0 && (risk === 'low' || risk === 'none')) {
                    setModerationResult('approved');
                    setModerationMessage(result?.recommended_fix || null);
                    const toastLang = aiLanguage === 'auto' ? result.language : aiLanguage;
                    showToast(toastLang === 'vi' ? 'Nội dung an toàn!' : 'Content looks safe!', 'success');
                } else {
                    setModerationResult('flagged');
                    const msg = result?.recommended_fix || (issues.length ? `Found issues: ${issues.join(', ')}` : 'Potential issues found.');
                    setModerationMessage(msg);
                    const toastLang = aiLanguage === 'auto' ? result.language : aiLanguage;
                    showToast(toastLang === 'vi' ? 'Phát hiện vấn đề — kiểm tra chi tiết trong bảng thông báo.' : 'Potential issues found. See details.', 'error');
                    console.warn('Moderation flagged:', result);
                }
            }
      setIsAiLoading(false);
  };
    ...
    <div className="flex h-[calc(100vh-65px)] overflow-hidden bg-white text-slate-800 font-sans">
      ...
      {showAiPanel && (
          <div className="w-80 bg-white border-l border-slate-200 flex flex-col flex-shrink-0 shadow-xl z-20">
              <div ref={aiPanelRef} className="p-5 border-b border-slate-100 bg-purple-50/30 flex items-center justify-between">
                  <h3 className="font-bold text-indigo-950 flex items-center gap-2">
                      <Wand2 className="w-5 h-5 text-purple-600" />
                      AI Muse
                  </h3>
                  <div className="flex items-center gap-2">
                      <label className="text-xs text-slate-500 mr-2">Lang:</label>
                      <div className="flex items-center bg-white rounded-full p-0.5 border border-slate-200">
                          <button onClick={() => setAiLanguage('auto')} className={`px-3 py-1 rounded-full text-xs ${aiLanguage === 'auto' ? 'bg-purple-600 text-white' : 'text-slate-600'}`}>AUTO</button>
                          <button onClick={() => setAiLanguage('vi')} className={`px-3 py-1 rounded-full text-xs ${aiLanguage === 'vi' ? 'bg-purple-600 text-white' : 'text-slate-600'}`}>VI</button>
                          <button onClick={() => setAiLanguage('en')} className={`px-3 py-1 rounded-full text-xs ${aiLanguage === 'en' ? 'bg-purple-600 text-white' : 'text-slate-600'}`}>EN</button>
                                            </div>
                                            <div ref={suggestionRef}>
                                            {aiSynopsis && (
                                                <div className="p-3 bg-purple-50 border border-purple-100 rounded-md text-sm text-slate-700">{aiSynopsis}</div>
                                            )}
                                            {aiTitles && aiTitles.length > 0 && (
                                                <div className="p-3 bg-white border border-slate-100 rounded-md text-sm space-y-1">
                                                    <div className="text-xs font-bold text-slate-400">Title suggestions</div>
                                                    {aiTitles.map((t, i) => <div key={i} className="text-sm">{t}</div>)}
                                                </div>
                                            )}
                                            {aiCharacterCheck && aiCharacterCheck.issues.length > 0 && (
                                                <div className="p-3 bg-red-50 border border-red-100 rounded-md text-sm text-red-700">
                                                    <div className="font-bold">Character Issues</div>
                                                    <div className="text-xs mt-1">{aiCharacterCheck.explanation}</div>
                                                    <ul className="mt-2 list-disc pl-5 text-xs">
                                                        {aiCharacterCheck.issues.map((it, idx) => <li key={idx}>{it}</li>)}
                                                    </ul>
                                                </div>
                                            )}
                                            </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-8">
                  ...
                  <div className="space-y-4">
                      <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Spark Creativity</h4>
                                            <div className="flex flex-col gap-2 mb-2">
                                                    <input id="ai-keywords" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-slate-900 text-sm" placeholder="keywords (comma-separated)" />
                                                    <div className="flex gap-2">
                                                        <button onClick={() => handleGenerateSynopsis((document.getElementById('ai-keywords') as HTMLInputElement)?.value)} disabled={isAiLoading} className="px-3 py-2 bg-white text-purple-600 hover:bg-purple-50 rounded-lg text-sm font-bold border border-purple-100">Generate Synopsis</button>
                                                        <button onClick={handleSuggestTitles} disabled={isAiLoading} className="px-3 py-2 bg-white text-slate-600 hover:bg-slate-50 rounded-lg text-sm font-bold border border-slate-100">Suggest Titles</button>
                                                        <button onClick={handleCharacterCheck} disabled={isAiLoading} className="px-3 py-2 bg-white text-red-600 hover:bg-red-50 rounded-lg text-sm font-bold border border-red-100">Character Check</button>
                                                    </div>
                                            </div>
                  ...
                  <div className="space-y-4">
                      ...
                      {moderationResult === 'flagged' && (
                          <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex flex-col gap-3 text-red-700 text-sm font-medium">
                              <div className="bg-red-200 p-1 rounded-full"><XCircle className="w-4 h-4" /></div> Issues Found
                              {moderationMessage && (
                                <div className="text-xs text-slate-700 bg-white/50 p-3 rounded-md border border-slate-100 mt-2">{moderationMessage}</div>
                              )}
                          </div>
                      )}
                  </div>
