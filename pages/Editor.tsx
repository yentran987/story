...
export const Editor: React.FC<EditorProps> = ({ setView, onBack, showToast, initialStory, onSaveStory }) => {
  ...
  const handleGenerateIdea = async (type: 'twist' | 'rewrite') => {
    ...
    try {
      ...
      if (!result) {
        const errorMsg = aiLanguage === 'vi' 
          ? 'AI không khả dụng. Vui lòng kiểm tra cấu hình API key.' 
    ...
    try {
      ...
      if (!result) {
        ...
        if (result.status === 'approved') {
          ...
          const toastMsg = result.language === 'vi' ? 'Nội dung an toàn!' : 'Content looks safe!';
          showToast(toastMsg, 'success');
        } else {
          setModerationResult('flagged');
          setModerationMessage(result.message || null);
          const toastMsg = result.language === 'vi' 
            ? 'Phát hiện vấn đề — kiểm tra chi tiết trong bảng thông báo.' 
            : 'Potential issues found. See details.';
      ...
      showToast('Failed to check content', 'error');
    } finally {
      setIsAiLoading(false);
    }
  };
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button 
                    ...
                    className="px-4 py-2 bg-white text-purple-600 hover:bg-purple-50 rounded-lg text-sm font-bold border border-purple-100 flex items-center gap-2 shadow-sm transition-all disabled:opacity-50"
                  >
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-8">
            ...
            <div className="space-y-4">
              ...
              {moderationResult === 'flagged' && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex flex-col gap-3 text-red-700 text-sm font-medium">
                  ...
                  {moderationMessage && (
                    <div className="text-xs text-slate-700 bg-white/50 p-3 rounded-md border border-slate-100">{moderationMessage}</div>
                  )}
                </div>
              )}
            </div>
            ...
            {aiSuggestion && (
              <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 animate-fade-in-up shadow-sm">
                ...
                <p className="text-sm text-slate-700 italic mb-4 font-serif leading-relaxed whitespace-pre-wrap">"{aiSuggestion}"</p>
                <button 
                  onClick={() => {
