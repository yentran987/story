...
export const Reader: React.FC<ReaderProps> = ({ story, initialChapterIndex, onSaveProgress, onBack }) => {
  ...
  useEffect(() => { setAiSummary(null); }, [currentChapterIndex]);

    const handleSummarize = async () => {
        setIsLoading(true);
        try {
            // Use summarizeText from the geminiService (legacy file you provided) which returns a string
            const res = await geminiService.summarizeText(currentChapter.content);
            setAiSummary(res || '');
        } catch (err) {
