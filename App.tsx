...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await storiesService.listStories();
        if (!mounted) return;

        if (!list || list.length === 0) {
          // No rows returned — likely empty DB or permission issue. Use mock data fallback
          setStories(MOCK_STORIES);
          setStoriesError('Unable to load stories from the database (no rows or permission denied). Showing local sample stories instead. Check your Supabase table policies / anon key permission.');
        } else {
          setStories(list);
        }
      } catch (e: any) {
        console.error('Failed to load stories', e);
        // Provide helpful message for RLS / permission issues
        const message = e?.message || String(e);
        setStoriesError(`Failed to load stories from the database: ${message}. Check your Supabase table policies / anon key permission.`);
        // Fallback to sample data so the app remains usable
        setStories(MOCK_STORIES);
      } finally {
  ...
  const handleSelectStory = (story: Story) => {
    (async () => {
      try {
        const full = await storiesService.getStory(story.id);
  ...
  const handleEditStory = (story: Story) => {
    (async () => {
      try {
        // Try to fetch full story (with chapters) from DB; fall back to the passed story
        const full = await storiesService.getStory(story.id);
        setStoryToEdit(full || story);
      } catch (e) {
        console.error('Failed to fetch story for edit', e);
        showToast('Could not load full story from database. Editing with local data.', 'error');
  ...
  const handleSaveStoryFromEditor = (updatedStory: Story) => {
    (async () => {
        try {
          // determine if this is a new story: treat empty or non-numeric or 0 as new
          const idNum = Number(updatedStory.id);
          const isNew = !updatedStory.id || Number.isNaN(idNum) || idNum <= 0;
          if (isNew) {
          const newId = await storiesService.createStory(updatedStory);
          // attach returned id
          const created = { ...updatedStory, id: String(newId) };
          setStories(prev => [created, ...prev]);
          showToast('New story created!', 'success');
        } else {
          await storiesService.updateStory(updatedStory);
          setStories(prev => prev.map(s => (s.id === updatedStory.id ? updatedStory : s)));
          showToast('Story updated successfully!', 'success');
        }
        } catch (e: any) {
          console.error('Failed to save story', e);
          // Surface more helpful error for DB issues
          const errMsg = e?.message || JSON.stringify(e);
          const advice = /permission|authenticated|row-level security|RLS/i.test(errMsg)
            ? '. This is often caused by Supabase Row-Level Security (RLS) or missing permissions for the anon key. Check your table policies or authenticate the user.'
            : '';
          showToast(`Failed to save story: ${errMsg}${advice}`, 'error');
      } finally {
        navigateTo(View.AUTHOR_DASHBOARD);
      }
    })();
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-purple-200 selection:text-purple-900">
      {/* DB access warning */}
      {storiesError && (
        <div className="fixed inset-x-6 top-6 z-50">
          <div className="max-w-7xl mx-auto bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg p-3 shadow-sm font-medium text-sm">
            <strong>Database notice:</strong> {storiesError}
            <div className="mt-2 flex items-center gap-3">
              <button onClick={() => setShowDbHelp(prev => !prev)} className="text-xs underline text-slate-700">{showDbHelp ? 'Hide suggested fixes' : 'Show suggested fixes'}</button>
              <span className="text-[11px] text-slate-500">(Click to show troubleshooting steps)</span>
            </div>
            {showDbHelp && (
              <div className="mt-3 bg-white p-3 rounded-md border border-slate-100 text-xs text-slate-600">
                <div className="font-semibold mb-1">Quick fixes:</div>
                <ol className="list-decimal pl-5 space-y-1">
                  <li>Open Supabase dashboard → Auth &gt; API and verify the <code>anon</code> key is the one used in <code>services/supabase.ts</code>.</li>
                  <li>In Supabase dashboard go to <strong>Database &gt; Policies</strong> for <code>stories</code> and <code>chapters</code>. If Row-Level Security (RLS) is enabled, add policies to allow <code>SELECT</code> (and <code>INSERT/UPDATE/Delete</code> for authenticated users) or temporarily disable RLS for development.</li>
                  <li>Alternatively, sign in the user before performing writes so policies that require authentication pass.</li>
                </ol>
                <div className="mt-2 text-[11px] text-slate-400">See Supabase docs: https://supabase.com/docs/guides/auth/row-level-security</div>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16 items-center">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={() => { setHistory([]); setView(View.LANDING); }} className="text-slate-500 hover:text-indigo-600 transition-colors text-sm font-bold uppercase tracking-wide">Exit Studio</button>
